    ...
    <div style={{ padding: '1.5rem', maxWidth: '1200px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 360px' : '1fr', gap: '1rem' }}>
        ...
        <div style={{ background: '#111', border: '0.5px solid #222', borderRadius: '12px', overflow: 'hidden' }}>
          {loading ? (
            <div style={{ padding: '3rem', textAlign: 'center', color: '#555', fontSize: '12px' }}>Loading reviews...</div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: '3rem', textAlign: 'center' }}>
              <div style={{ fontSize: '13px', fontWeight: '500', color: '#1D9E75', marginBottom: '6px' }}>No reviews yet</div>
              <div style={{ fontSize: '12px', color: '#555' }}>Reviews from your promo contacts will appear here.</div>
            </div>
          ) : (
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: '10px 14px' }}>
                        <div style={{ fontWeight: '500', color: '#fff', fontSize: '12px' }}>{r.contact_name ?? '—'}</div>
                        <div style={{ fontSize: '10px', color: '#555' }}>{r.contact_type ?? ''}</div>
                      </td>
                      <td style={{ padding: '10px 14px' }}>
                        <div style={{ fontSize: '12px', color: '#888' }}>{r.release_title ?? '—'}</div>
                        <div style={{ fontSize: '10px', color: '#444', fontFamily: 'monospace' }}>{r.release_catalogue ?? ''}</div>
                      </td>
                      ...
                      <td style={{ padding: '10px 14px' }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', gap: '5px' }}>
                          {r.status === 'pending' && (
                            <>
                              <button onClick={() => approve(r.id)} style={{ padding: '3px 8px', background: '#0a2a1e', border: '0.5px solid #1a4a3a', borderRadius: '6px', color: '#4ecca3', fontSize: '11px', cursor: 'pointer' }}>Approve</button>
                              <button onClick={() => { setShowRejectModal(r.id); setRejectReason('') }} style={{ padding: '3px 8px', background: '#2a0a0a', border: '0.5px solid #5a1a1a', borderRadius: '6px', color: '#f08080', fontSize: '11px', cursor: 'pointer' }}>Reject</button>
                            </>
                          )}
                          <button onClick={() => toggleFeatured(r.id, r.is_featured)} style={{ padding: '3px 8px', background: 'transparent', border: '0.5px solid #333', borderRadius: '6px', color: r.is_featured ? '#b8b4f0' : '#555', fontSize: '11px', cursor: 'pointer' }}>
          ...
          <div style={{ background: '#111', border: '0.5px solid #222', borderRadius: '12px', padding: '1.25rem', alignSelf: 'start' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '1rem' }}>
              <div style={{ background: '#1a1a1a', borderRadius: '8px', padding: '0.6rem 0.75rem' }}>
                <div style={{ fontSize: '10px', color: '#555', marginBottom: '3px' }}>Rating</div>
                <div style={{ fontSize: '16px', color: '#f5c842', letterSpacing: '1px' }}>{stars(selected.rating)}</div>
              </div>
              <div style={{ background: '#1a1a1a', borderRadius: '8px', padding: '0.6rem 0.75rem' }}>
                <div style={{ fontSize: '10px', color: '#555', marginBottom: '3px' }}>Status</div>
                <div style={{ fontSize: '14px', fontWeight: '500', color: STATUS_COLORS[selected.status].color }}>{selected.status}</div>
              </div>
            </div>

            <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Release</div>
            <div style={{ padding: '8px', background: '#1a1a1a', borderRadius: '6px', marginBottom: '1rem', fontSize: '12px' }}>
              <div style={{ color: '#fff' }}>{selected.release_title}</div>
              <div style={{ color: '#444', fontFamily: 'monospace', fontSize: '11px', marginTop: '2px' }}>{selected.release_catalogue}</div>
            </div>

            {selected.body && (
              <>
                <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Review</div>
                <div style={{ padding: '8px', background: '#1a1a1a', borderRadius: '6px', marginBottom: '1rem', fontSize: '12px', color: '#888', lineHeight: '1.5' }}>
                  {selected.body}
                </div>
              </>
            )}
            ...
            {selected.charted && selected.chart_name && (
              <>
                <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Chart</div>
                <div style={{ padding: '8px', background: '#0a2a1e', borderRadius: '6px', marginBottom: '1rem', fontSize: '12px', color: '#4ecca3' }}>
                  {selected.chart_name}
                </div>
              </>
            )}
            ...
            {selected.rejected_reason && (
              <>
                <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Rejection reason</div>
                <div style={{ padding: '8px', background: '#2a0a0a', borderRadius: '6px', marginBottom: '1rem', fontSize: '12px', color: '#f08080' }}>
                  {selected.rejected_reason}
                </div>
              </>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {selected.status === 'pending' && (
                <>
                  <button onClick={() => approve(selected.id)} style={{ padding: '8px', background: '#1D9E75', border: 'none', borderRadius: '8px', color: '#fff', fontSize: '12px', fontWeight: '500', cursor: 'pointer' }}>Approve review</button>
                  <button onClick={() => { setShowRejectModal(selected.id); setRejectReason('') }} style={{ padding: '8px', background: '#2a0a0a', border: '0.5px solid #5a1a1a', borderRadius: '8px', color: '#f08080', fontSize: '12px', cursor: 'pointer' }}>Reject review</button>
                </>
              )}
              <button onClick={() => deleteReview(selected.id)} style={{ padding: '8px', background: 'transparent', border: '0.5px solid #2a1a1a', borderRadius: '8px', color: '#5a2a2a', fontSize: '12px', cursor: 'pointer' }}>Delete review</button>
            </div>
          </div>
        )}
      </div>
      ...
      {showRejectModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}
          ...
          <div style={{ background: '#111', border: '0.5px solid #333', borderRadius: '12px', padding: '1.5rem', width: '400px' }}
            onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: '14px', fontWeight: '500', marginBottom: '1rem' }}>Reject review</div>
            <div style={{ fontSize: '11px', color: '#666', marginBottom: '4px' }}>Reason (optional)</div>
            <textarea
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
              <button onClick={() => reject(showRejectModal)} style={{ padding: '8px 20px', background: '#5a1a1a', border: 'none', borderRadius: '8px', color: '#f08080', fontSize: '12px', fontWeight: '500', cursor: 'pointer' }}>Reject</button>
              <button onClick={() => setShowRejectModal(null)} style={{ padding: '8px 16px', background: 'transparent', border: '0.5px solid #333', borderRadius: '8px', color: '#666', fontSize: '12px', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
